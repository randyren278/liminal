...
| Passive acoustic organ (`liminal-capture`): mic tap + RMS/peak/ZCR/spectral centroid-rolloff-flatness/VAD-heuristic features, no MFCC, zero raw audio persisted | EXPERIMENTAL — same reason as the Vision organ above, plus: `voice_activity_probability` is an explicit heuristic (§28), not a trained model — see `app/Liminal/Sources/LiminalCore/AudioFeatures.swift`'s doc comment for its known limitations | `app/Liminal` (`liminal-capture` binary) |
| Wi-Fi organ (`liminal-capture`): live Mode A scanning, no SSID/BSSID field exists on the emitted type | EXPERIMENTAL — builds and unit-tests clean; live scanning has not been confirmed by a human running it | `app/Liminal` (`liminal-capture` binary) |
| Bluetooth organ (`liminal-capture`): live proximity scanning, Keychain-persisted HMAC pseudonyms per §18/§39 | EXPERIMENTAL — same reason as Wi-Fi, plus the Keychain-backed key storage isn't unit-tested (CI can't guarantee an unlocked keychain) | `app/Liminal` (`liminal-capture` binary) |
