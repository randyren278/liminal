...
| `liminal-tui` | `1 LIVE FIELD`, `2 BELIEF`, `3 MEMORY`, `4 NOTES`, `5 POSE`, and `6 CALIBRATE`; live SQLite polling; telemetry-driven bitmap field using all derived feature values; recent observation rates; derived pose skeleton; nonvisual vision-off path; pause/resume; diagnostics; displayed belief evidence IDs; bounded persisted Tier-0 drafts; historical record/provenance drilldown; explicit synthetic demo mode | Workspace tests pass; a fresh direct Terminal run rendered live telemetry from the launcher, added persisted observations, and exited cleanly. The demo renderer remains explicitly separate from the acceptance capture. |
| Runtime launcher | Starts daemon, capture, and TUI together; waits for daemon/capture readiness; recursively cleans child processes and temporary logs | `scripts/run-liminal.sh` live run added observations and explicit gap events, then left no Liminal processes; daemon startup owns stale-socket recovery |
...
These are intentionally not claimed by the current TUI or its synthetic/unit
test evidence.
